...
class TranslaterApp(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.pack()
        # 设置frame
        text_frame = tk.Frame(self, borderwidth=1, relief='sunken',width=500, height=600)
        text_frame.pack()
        ...
        # 文字输入
        label0 = tk.Label(text_frame, text="待翻译文字：")
        label0.grid(row=1, column=0, sticky=tk.W)
        self.input_text = tk.Text(text_frame, height=8)
        self.input_text.grid(row=1, column=1, columnspan=3)
        # 有道翻译按钮
        self.button = tk.Button(text_frame, text="有道翻译", command=self.resultsText)
        self.button.grid(row=2, column=1, sticky=tk.W)
        #百度翻译
        self.baiduApi_bt = tk.Button(text_frame, text="百度翻译", command=self.resultsText)
        self.baiduApi_bt.grid(row=2, column=2, sticky=tk.W)
        # 显示翻译结果
        label1 = tk.Label(text_frame, text='翻译结果：')
        label1.grid(row=3, column=0, sticky=tk.W)
        self.results_text = tk.Text(text_frame, wrap=tk.WORD, height=8)
        self.results_text.grid(row=3, column=1, sticky=tk.W, columnspan=3)
